...
class AdminUserMoneyHistoryController extends WireTablePopupForms
{
    public function __construct()
    {
        ...
        $this->actions['view_list'] = "jiny-shop-account::admin.user_money_history.list";
        $this->actions['view_form'] = "jiny-shop-account::admin.user_money_history.form";
    ...
    public function hookCreating($wire, $value)
    {
        ...
        if($user) {
            ...
            $money = DB::table('user_money')->where('user_id',$nesteds['userid'])->first();
            if($money) {
    ...
    public function hookStored($wire, $form)
    {
        $nesteds = $wire->actions['nesteds'];
        //dd($form['balance']);
        DB::table('user_money')->where('user_id',$nesteds['userid'])->update([
